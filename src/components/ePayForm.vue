...
export default {
  ...
  data() {
    const data = {
      parseData: {
        'ownerName': this.$store.state.sources.tips.data.property_info.owner_name,
        'accountNum': this.$store.state.sources.tips.data.brt_no,
        'totalDue': '',
        'balances': this.$store.state.sources.tips.data.tax_years,
        'address': {
  ...
  methods: {
    calculateTotalDue() {
      return this.parseData.balances.reduce((acc, year) => {
